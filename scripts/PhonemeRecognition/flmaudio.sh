...
python -m torch.distributed.launch --nproc_per_node=4  --master_port=29606 run_downstream.py \
-m train \
-u flmaudio \
...
-d ctc \
-c downstream/ctc/libriphone.yaml \
-n PR_FLM_600000_lr1e2_16layers \
...
PYTHONPATH=$PROJECT_PATH \
LD_LIBRARY_PATH=/usr/local/cuda/compat/lib:/usr/local/nvidia/lib:/usr/local/nvidia/lib64 \
python -m torch.distributed.launch --nproc_per_node=2  --master_port=29607 run_downstream.py \
-m train \
-u flmaudio \
-k /share/project/lx/projects/NativeAudio-trainer-ddp/outputs/ckpts/audio_model_multi_node_mix/checkpoint-step-600000 \
-g /share/project/lx/projects/NativeAudio-trainer-ddp/audio_models/config_mini.json \
-d ctc \
-c downstream/ctc/libriphone.yaml \
-n PR_FLM_600000_lr1e2_16layers_drop30 \
--upstream_feature_selection hidden_state_16