...
# >>>>>>>>>>> vit modeling (copy from https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2_vl/modeling_qwen2_vl.py)
...
class Qwen2_5_VLPatchMerger(nn.Module):
    def __init__(self, dim: int, context_dim: int, spatial_merge_size: int = 2) -> None:
        ...
        self.ln_q = TeleFLMRMSNorm(context_dim, eps=1e-6)
...
class Qwen2_5_VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        super().__init__()
        self.norm1 = TeleFLMRMSNorm(config.hidden_size, eps=1e-6)
        self.norm2 = TeleFLMRMSNorm(config.hidden_size, eps=1e-6)
...
class TeleFLMDecoderLayer(nn.Module):
    def __init__(self, config: TeleFLMConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = TELEFLM_ATTENTION_CLASSES.get(
            config._attn_implementation, TeleFLMAttention
        )(config=config, layer_idx=layer_idx)
        self.mlp = TeleFLMMLP(config)
        self.input_layernorm = TeleFLMRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = TeleFLMRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class MultiModalEmbedding(nn.Module):
    def __init__(self, config):
        ...
        self.audio_embeddings = torch.nn.ModuleList(
            [
                torch.nn.Embedding(self.aud_vocab_size, self.hidden_size)
...
class TeleFLMModel(TeleFLMPreTrainedModel):
    ...
    def __init__(self, config: TeleFLMConfig):
        ...
        self.layers = nn.ModuleList(
            [
                TeleFLMDecoderLayer(config, layer_idx)
    ...
    """
     def forward(self, text_ids, position_ids, attention_mask,
                speak_ids, listen_ids, image_ids, video_ids,
                text_ids_labels=None, speak_ids_labels=None, image_ids_labels=None, video_ids_labels=None,
                retriever_input_ids=None,
                retriever_position_ids=None,
                retriever_attn_mask=None,
                tokentype_ids=None, inference_params=None):

        text_ids            [batch x max_length]
        text_ids_labels	    [batch x max_length]

        speak_ids	        [batch x max_length x 8]
        speak_ids_labels	[batch x max_length x 8]

        listen_ids	        [batch x max_length x 8]

        image_ids	        [batch x max_length x 2]
        image_ids_labels	[batch x max_length x 2]

        video_ids	        [batch x max_length x 2]
        video_ids_labels	[batch x max_length x 2]
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (audio_ids is None) ^ (inputs_embeds is not None):
            raise ValueError(
                "You cannot specify both audio_ids and inputs_embeds at the same time, and must specify either one"
            )
...
@dataclass
class CausalAudioModelOutput(ModelOutput):
    # losses: Optional[Dict[torch.FloatTensor]] = None
    loss: Optional[torch.FloatTensor] = None
    channel_losses: Optional[List[torch.FloatTensor]] = None
...
class TeleFLMForCausalLM(TeleFLMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        depth_config = DepthGPTConfig(
            ...
            pad_token_id=config.aud_emp_token_id,
    ...
    def _forward_audio(self, transformer_output_states, audio_input_ids):
    ...
    def forward(
        ...
        **kwargs,
        ...
        aud_input_hidden_state = outputs.last_hidden_state[:, :-1, :]
        aud_input_audio_ids = audio_ids[:, 1:, :-1]

        audio_logis = self.aud_output_layers(
            aud_input_hidden_state.reshape((-1, aud_input_hidden_state.shape[-1])),
            aud_input_audio_ids.reshape((-1, audio_ids.shape[-1] - 1))
        )
        audio_logis = audio_logis.reshape(
            (
        ...
        if labels is not None:
            loss, channel_losses = self.compute_loss(audio_logis, labels)
        else:
            loss, channel_losses = self.compute_loss(audio_logis, audio_ids[:, 1:, :])
        # return self._forward_text(outputs, labels, return_dict)
        # return self.forward_audio(outputs.last_hidden_state, )
        return CausalAudioModelOutput(
            loss=loss,
    ...
    def compute_loss(
        ...
        # print(f"[XXX] loss1={loss.shape}")
        # loss = [b s c]
        token_loss = token_loss.view(batch_size, seq_length, num_channel)
        # print(f"token_loss={token_loss.shape}, _mask={loss_mask.shape}, _mask_sum={loss_mask}")
